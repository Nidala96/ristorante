...
public class PiattoService {
    ...
    @Transactional
    public ResponseEntity<?> addPiatto(UserDetails principal, String nome, Set<Long> listaIngredientiId) {
    ...
    @Transactional
    public ResponseEntity<?> eliminaPiatto(UserDetails principal, Long piattoId) {
        Utente utente = (Utente) principal;
        if(utente == null)
            return  new ResponseEntity<>("Utente non esiste", HttpStatus.NOT_FOUND);
        if(!ristoranteRepository.existsByEmail(utente.getEmail()))
